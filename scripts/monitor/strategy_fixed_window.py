...
def check_buy_dip_signal(sina, stock_code, tongguo_date):
    ...
    # 固定窗口：上市后 25-35 天
    if days_since_tongguo < 25 or days_since_tongguo > 35:
        return None
...
def check_signals():
    ...
    for b in bonds:
        ...
        if signal and signal['has_signal']:
            alert_key = f'{stock_code}_fixed_window'
    ...
    for b in bonds:
        ...
        status = '⏰' if 25 <= days_since <= 35 else '📋'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='固定窗口策略监控')
