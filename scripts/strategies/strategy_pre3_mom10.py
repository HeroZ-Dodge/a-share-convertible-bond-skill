...
def backtest(cache, bonds=None, limit=100):
    ...
    for b in bonds:
        ...
        pool.append({
            ...
            'd9_price': d9_price, 'd9_ret': d9_ret,
    ...
    for v in sample:
        ...
        if ri >= 10:
            p10 = prices[sd[ri - 10]]['close']
            if p10 > 0: mom10 = ((reg_price - p10) / p10) * 100
        if pre3_ret <= 2 and mom10 < 5:
            triggered.append({**v, 'pre3': round(pre3_ret, 2), 'mom10': round(mom10, 2)})
...
def _report(triggered, sample):
    ...
    for t in triggered:
        ...
        print(f"  {marker} {t['name']:>12} {t['reg_date']}  买入价{t['buy_price']:.2f} → D+9价{t['d9_price']:.2f}  {t['d9_ret']:>+5.1f}%  "
              f"pre3={t['pre3']:+.1f}% mom10={t['mom10']:+.1f}%")
