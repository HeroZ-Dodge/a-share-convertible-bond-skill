...
# -*- coding: utf-8 -*-
"""
注册策略 1：注册前3天微涨(≤2%) + 10日弱势(<5%) + 注册日大涨(>0%)
"""
import sys, os, re
...
def check(bond_info, cache, today_str=None):
    ...
    Args:
        bond_info: dict with 'anchor_date' (YYYY-MM-DD)
        cache: BacktestCache instance
        today_str: optional today date string
    Returns:
        (bool, dict) - 是否满足条件 + 详细数据
    """
    ...
    ok = pre3_ret <= 2 and mom10 < 5 and rc > 0
    return ok, {
        'pre3': round(pre3_ret, 2),
        'mom10': round(mom10, 2),
        'rc': round(rc, 2),
...
def backtest(cache, bonds=None, limit=100):
    ...
    Args:
        cache: BacktestCache instance
        bonds: optional list of bond dicts (from get_jisilu_bonds)
        limit: max bonds to test
    Returns:
        dict with stats
    """
    if bonds is None:
    ...
    for v in sample:
        ...
        if pre3_ret <= 2 and mom10 < 5 and rc > 0:
            triggered.append({**v, 'pre3': round(pre3_ret, 2), 'mom10': round(mom10, 2), 'rc': round(rc, 2)})
...
def _report(triggered, sample):
    ...
    for t in triggered:
        ...
        print(f"  {marker} {t['name']:>12} {t['reg_date']}  买入价{t['buy_price']:.2f} → D+9价{t['d9_price']:.2f}  {t['d9_ret']:>+5.1f}%  "
              f"pre3={t['pre3']:+.1f}% mom10={t['mom10']:+.1f}% rc={t['rc']:+.1f}%")
