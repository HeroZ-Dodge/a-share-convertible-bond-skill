...
# -*- coding: utf-8 -*-
"""
B1 策略每日监控 — 注册日因子 + 缩量买入 + 动态退出

策略逻辑:
  买入: 注册日因子满足 S1 (pre3≤2% + mom10<5% + rc>0%) + vol_ratio<0.8 → D+1 开盘买入
  卖出: 持仓期间每日检查, TP5/SL5 (盈利≥5%止盈 / 亏损≥5%止损) / 10天超时退出

  所有买入决策基于注册日收盘价计算的因子，注册日收盘后可判断，D+1 开盘买入，合法可执行。

用法:
  python3 scripts/monitor_b1_strategy.py              # 监控今日机会
  python3 scripts/monitor_b1_strategy.py --all-bonds  # 显示所有注册转债(含不在窗口的)
  python3 scripts/monitor_b1_strategy.py --detail     # 显示每只债的详细因子
"""
import sys, os, re
...
def get_factors(cache, sc, anchor):
    ...
    day_offset = today_idx - ri  # 当前是注册后第几天
...
def main():
    ...
    # 分析每只债
    buy_candidates = []      # 明天可以买的
    holding = []             # 当前持仓中
    missed = []              # 超出窗口的
    no_signal = []           # 有注册但不在买入窗口/不满足因子
    ...
    for sc, anchor, name, bond_info in registered:
        ...
        # 判断买入条件 (S1 + B1)
        s1_ok = factors['pre3'] <= 2 and factors['mom10'] < 5 and factors['rc'] > 0
        b1_ok = s1_ok and factors['vol_ratio'] < 0.8
        ...
        if day_off < 0:
            ...
            if b1_ok:
                buy_candidates.append({
                    ...
                    'factors': factors, 'level': 'B1',
                })
            elif s1_ok:
                buy_candidates.append({
                    'name': name, 'code': sc, 'anchor': anchor,
                    'factors': factors, 'level': 'S1',
                })
        elif 1 <= day_off <= 10:
            # 在持仓窗口
    ...
    if buy_candidates:
        print(f"\n  {'名称':<12} {'代码':>8} {'注册日':<12} {'策略':>4} "
              f"{'pre3':>6} {'mom10':>7} {'rc':>6} {'vol比':>6} {'D+1开盘':>8}")
        print("  " + "-" * 80)
        for item in sorted(buy_candidates, key=lambda x: x['anchor'], reverse=True):
            ...
            print(f"  {item['name']:<12} {item['code']:>8} {item['anchor']:<12} "
                  f"{'★' if level=='B1' else ' '}{level:>3} "
                  f"{f['pre3']:>+5.1f}% {f['mom10']:>+6.1f}% {f['rc']:>+5.1f}% "
                  f"{f['vol_ratio']:>5.2f} {buy_str:>8}")
    else:
        print("\n  当前无买入信号")
    ...
    if holding:
        ...
        for item in sorted(holding, key=lambda x: x['ret'], reverse=True):
            ...
            level_str = 'B1' if item['b1_ok'] else ('S1' if item['s1_ok'] else '')
    ...
    # --- 汇总 ---
    print("\n" + "=" * 90)
    buy_b1 = sum(1 for i in buy_candidates if i['level'] == 'B1')
    buy_s1 = sum(1 for i in buy_candidates if i['level'] == 'S1')
    need_exit = sum(1 for i in holding if i['exit_reason'])
    print(f"汇总: 已注册 {len(registered)} | "
          f"买入建议 {len(buy_candidates)} (B1={buy_b1} S1={buy_s1}) | "
