...
def backtest(cache, bonds=None, limit=100):
    ...
    for b in bonds:
        ...
        pool.append({
            ...
            'd9_price': d9_price, 'd9_ret': d9_ret, 'rc': rc,
    ...
    triggered = [v for v in sample if v['rc'] < -2]
...
def _report(triggered, sample):
    ...
    for t in triggered:
        ...
        print(f"  {marker} {t['name']:>12} {t['reg_date']}  买入价{t['buy_price']:.2f} → D+9价{t['d9_price']:.2f}  {t['d9_ret']:>+5.1f}%  rc={t['rc']:+.1f}%")
