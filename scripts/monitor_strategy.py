...
def init_backfill(cache: BacktestCache, monitor_db: MonitorDB,
    ...
    # 创建持仓
    positions = create_positions_for_registrations(new_regs, cache, monitor_db)

    # 处理已到期的买入信号
    buy_result = process_buy_signals(
        monitor_db, cache, today, quality_evaluator, min_rating)

    # 处理已到期的卖出信号
    sells = process_sell_signals(monitor_db, cache, today)

    return {
        'new_registrations': len(new_regs),
        'positions_created': len(positions),
        'buys_executed': len(buy_result['executed']),
        'buys_missed': len(buy_result['missed']),
...
def main():
    ...
    if args.init_backfill:
        ...
        print(f'   买入执行: {result.get("buys_executed", 0)} 只')
        print(f'   买入跳过: {result.get("buys_missed", 0)} 只')
        print(f'   卖出执行: {result.get("sells_executed", 0)} 只')
        return
